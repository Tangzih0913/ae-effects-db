...
即時搜尋支援繁簡中文、英文別名與常見錯字修正，可依分類／來源／型態篩選並高亮關鍵字。排序包含可解釋的精選熱門、搜尋相關度、名稱、分類、來源，以及只採官方日期的最新排序；每張卡片會顯示熱門分數，詳情頁可查看分數組成。星號收藏可匯出／匯入，亦可勾選 2～4 個工具並排比較。詳細頁、比較清單和篩選條件都有可分享網址，手機版提供固定搜尋列與自動漸進載入。精選熱門是人工精選、來源權重、資料完整度、近期更新與策展順序組成的分數，不代表下載量或銷量。
...
資料或熱門度設定變動後，請執行 `python tools/build_index.py` 更新 `dist/`。熱門精選與權重集中在 `curation/popularity.json`，不需要修改前端程式。
