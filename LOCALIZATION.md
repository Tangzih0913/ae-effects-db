...
AE 內建效果若能由 Adobe 官方效果清單判定，卡片會另外顯示 **Adobe 官方分類**。日文介面採用 Adobe 日文文件中的分類名稱，例如「ブラー＆シャープ」、「カラー補正」與「ディストーション」。
...
- 若有轉址，只接受同檔名的原廠 canonical 文件轉址，不接受首頁或不同文件。

截至 2026-08-13，共驗證 103 個不重複的 Adobe／Maxon 日文官方網址，涵蓋資料庫 374 筆條目。沒有通過的頁面一律沿用資料列原本的官方網址，不在瀏覽器執行時猜測語系路徑。
