...
3. **效果說明與外觀**：目前三種介面都顯示人工整理的繁體中文原文；英文與日文介面會明確標示，避免把未審核的機器翻譯當成正式翻譯。

AE 內建效果會依 Adobe 最新效果清單中的效果名稱，另外顯示 **Adobe 官方分類**。目前 271 筆內建資料中，269 筆效果已精確對應；其餘 `Smart Mask Interpolation` 與 `Time-Reverse Keyframes` 是面板工具／指令，不是效果選單項目，因此明確排除。日文介面採用 Adobe 日文文件中的 22 組分類名稱，例如「ブラー＆シャープ」、「カラー補正」與「ディストーション」。
...
截至 2026-08-13，共驗證 108 個不重複的 Adobe／Maxon 日文官方網址，涵蓋資料庫 426 筆條目。Adobe 目前提供的日文「Matte Effects」頁會回傳 HTTP 404，因此相關 5 筆資料暫時沿用英文官方網址；其他沒有通過的頁面也一律沿用資料列原本的官方網址，不在瀏覽器執行時猜測語系路徑。
