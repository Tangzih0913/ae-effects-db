...
一次產一整個系列時最容易犯，而且做完等於白做：

```jsonc
// ❌ 24 個調色效果共用同一句話，只有名字不同
...
DeFog 是「去霧」——上面那個錯誤版本讓使用者搜「去霧」找不到它，搜「調色」則一次噴出
24 筆長得一樣的結果。**產完自我檢查兩件事**：

1. 把效果名遮掉，desc 還能不能分辨是哪個效果？不能 → 重寫。
2. tags 去掉效果名後，是否與同系列其他條目完全相同？相同 → 補俗名／外觀／用途同義詞。
   分類名（「轉場」「調色」）可以放但不算數，每筆至少要有 3 個分類名以外、真人會搜的詞。

跑 `python validate.py` 會自動抓這兩件事；一次加很多筆時請跑 `python validate.py --strict`，
它會把樣板化直接算成錯誤。
...
python validate.py --strict                  # 一次加很多筆時用：樣板化也算錯誤
```
