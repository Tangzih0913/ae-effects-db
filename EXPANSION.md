...
| 72 | Gumroad 持續掃描第一輪：`After Effects ScriptUI`／`JSXBIN`／`panel`／`plugin` 高訊號搜尋 | **Project Info List Exporter、RAFIKIT Toolbox、LetterBlack Gen AI**；官方 Gumroad 頁分別確認專案資料稽核／缺失外掛／Render Queue 清單、圖層／預合成／SRT 等四分頁 ScriptUI 工作流，以及 AE 2025+ CEP 的 AI 動畫與表達式預覽、批次處理及腳本修復 | Grid Layers、ImpactFX、Apex Control Pro、MessCtrl、Motion UI 等既有略過決策與 Mouse Pack、Pearelax 等既有條目未重複加入；其餘候選繼續逐頁核實 | 3 筆新增；最新總筆數 2,953 |
