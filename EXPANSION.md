...
| 72 | Gumroad 持續掃描第一輪：`After Effects ScriptUI`／`JSXBIN`／`panel`／`plugin` 高訊號搜尋 | **Project Info List Exporter**；官方 Gumroad 頁確認可搜尋與排序專案內合成、圖層、效果、缺失外掛、素材路徑、Expression、Mask 與 Render Queue 資料，並含專案素材整理與清理功能 | Grid Layers、ImpactFX、Apex Control Pro、MessCtrl、Motion UI 等既有略過決策與 Mouse Pack、Pearelax 等既有條目未重複加入；其餘候選繼續逐頁核實 | 1 筆新增；最新總筆數 2,951 |
