...
```bash
python search.py 發光 glow          # 中英關鍵字都吃，空格分隔＝OR
python search.py --cat transition 甩鏡
python search.py --list-cats        # 看有哪些分類
```

搜不到就換同義詞再試（見下表）。中文長詞（如「煙霧模擬」）會自動拆詞，不必自己拆。
...
3. **回答**：挑 3–5 個，每個講「名稱（來源）→ 為什麼適合 → 關鍵參數往哪調」，
...
- 標 ⚠ `unverified` 的條目是「查無官方說明、依檔名推測」，回答時要註明不確定。
